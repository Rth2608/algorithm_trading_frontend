...
const SignupPage: React.FC = () => {
    ...
    const handleSubmit = (event: React.FormEvent) => {
        ...
        console.log(`회원가입 요청: 닉네임: ${nickname}, 이메일: ${email}, 알림 수신: ${receiveEmail ? '예' : '아니오'}`);
    ...
    return (
        <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
            <div className={FORM_BG_CLASS}>
                ...
                {message && (
                    <div className={`mb-4 p-3 rounded-lg text-sm font-semibold ${message.includes('성공') ? 'bg-green-500/20 text-green-300 border border-green-400' : 'bg-red-500/20 text-red-300 border border-red-400'}`}>
                        {message}
                    </div>
                )}
